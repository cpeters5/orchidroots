...
def search_distribution(request):
    ...
    if subregion_id and (not region_id or region_id == str(0)):
        print(f"subregion_id: {subregion_id}")
        try:
    ...
    # Handle DataTable sort
    order_column_index = request.GET.get('order[0][column]', '0')
    order_direction = request.GET.get('order[0][dir]', 'asc')
    columns = ['speciesstat__best_image', 'binomial']  # must match DataTables order
    ...
    context = {
        ...
        'genera': genera,
        'genus': genus,
        'app': 'distribution',  # or replace with your app name
    }
