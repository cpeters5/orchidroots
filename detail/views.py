...
def xmyphoto(request, pid):
    ...
    if not role or role == 'pub':
        url = "%s?role=%s" % (reverse('display:information', args=(pid,)), role)
        return HttpResponseRedirect(url)
    else:
...
def xmyphoto_browse_spc(request):
    ...
    context = {'my_list': page_list, 'type': 'species',
               ...
               'role': role, 'brwspc': 'active', 'author': author, 'family': family,
...
def xmyphoto_browse_hyb(request):
    ...
    context = {'my_list': page_list, 'type': 'hybrid',
               'myspecies_list': myspecies_list, 'myhybrid_list': myhybrid_list,
               'role': role, 'brwhyb': 'active', 'author': author, 'family': family,
               'page_range': page_range, 'last_page': last_page, 'num_show': num_show, 'page_length': page_length,
               'page': page, 'first': first_item, 'last': last_item, 'next_page': next_page, 'prev_page': prev_page,
               'author_list': author_list,
               'title': 'myphoto_browse',
...
# Moved to orchidaceae
def ancestor(request, pid=None):
    role = getRole(request)
    family = 'Orchidaceae'
    if pid:
        send_url = '/orchidaceae/ancestor/' + str(pid) + '/?family=' + str(family) + '&role=' + role
    else:
        send_url = '/'
    return HttpResponseRedirect(send_url)
def ancestrytree(request, pid=None):
    ...
    family = 'Orchidaceae'
    if pid:
        send_url = '/orchidaceae/ancestrytree/' + str(pid) + '/?family=' + str(family) + '&role=' + role
    else:
        send_url = '/'
    return HttpResponseRedirect(send_url)
def progeny(request, pid=None):
    ...
    family = 'Orchidaceae'
    if pid:
        send_url = '/orchidaceae/progeny/' + str(pid) + '/?family=' + str(family) + '&role=' + role
    else:
        send_url = '/'
    return HttpResponseRedirect(send_url)
def progenyimg(request, pid=None):
    ...
    family = 'Orchidaceae'
    if pid:
        send_url = '/orchidaceae/progenyimg/' + str(pid) + '/?family=' + str(family) + '&role=' + role
    else:
        send_url = '/'
    return HttpResponseRedirect(send_url)
