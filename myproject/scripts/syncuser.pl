...
sub setAuthor {
    foreach(@user) {
        if (exists $author{$credit_name_id{$_}}) {
            if ($credit_name{$_}) {
                my $stmt = "update accounts_photographer set displayname = '$credit_name{$_}',
                                fullname = '$fullname{$_}', user_id = $_
                            where author_id = '$credit_name_id{$_}'";
                getASPM($stmt);
                print "Update photographer, $_ - set displayname = creditname = $credit_name{$_}\n";
            }
            else {
                my $stmt = "update accounts_photographer set displayname = '$fullname{$_}', fullname = '$fullname{$_}'
                            where author_id = '$credit_name_id{$_}'";
                getASPM($stmt);
