...
if ($ARGV[0]) {
	my $pid = $ARGV[0];
	my $stmt = "select seed_id, pollen_id from orchidaceae_hybrid where pid = $pid";
	&getASPM($stmt);
...
sub initHybrid {
    my $stmt = "select pid, seed_id, pollen_id from orchidaceae_hybrid where pid not in
    			(select distinct did from orchidaceae_ancestordescendant)";
    &getASPM($stmt);
    while (my @row = $sth->fetchrow_array()) {
        $pid{$row[0]}++;
        $row[1] = 0 if !$row[1];
